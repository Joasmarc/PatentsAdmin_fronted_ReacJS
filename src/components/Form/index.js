...
import { tranformGenderId, transforTypeId } from '../../constants';
import './style.css';

export const Form = ({urlServer, cancelAction, setStatus, status}) => {

    const [id_types, setId_types] = useState([])
    const [sex_types, setSex_types] = useState([])

    const {name, last_name, id, onChange, form} = useForm({
        name:'',
        last_name:'',
        type_id: '',
        id: '',
    ...
    const sendData = () =>{
        if(dataValidator(form)){
            if (postRequestAPI(urlServer, form)){
                form.sex = tranformGenderId[form.sex];
                form.type_id = transforTypeId[form.type_id];
